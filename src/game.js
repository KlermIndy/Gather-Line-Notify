...
const { API_KEY, SPACE_ID, MAP_ID, ME_ID, MAP_X, MAP_Y } = require('./config');
...
const initGame = () => {
  ...
  game.subscribeToEvent('playerChats', (data) => {
    const { senderName, contents, messageType } = data.playerChats;
    console.log('[Gather] playerChats:', data);
    sendToTelegram(`[${messageType}] ${senderName}: ${contents}`);
  });

  game.subscribeToEvent('playerWaves', (data, context) => {
    if (data.playerWaves.targetId === ME_ID) {
      const senderName = context.player?.name ?? context.playerId ?? 'Someone';
      console.log('[Gather] playerWaves:', data);
      sendToTelegram(`${senderName} waved to you${data.playerWaves.isReply ? ' (reply)' : ''}`);
    }
  });
